...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        for valid_action in self.valid_actions:
            Q_value = self.Q[state][valid_action]
    ...
    def get_maxQ_actions(self, state, maxQ):

        """ Get actions that meet the maximum Q-value for a given state. """
        ...
        for valid_action in self.valid_actions:
            ...
            if Q_value == maxQ:
                actions.add(valid_action)
        
        return actions


    def select_maxQ_action(self, state):
        """ Select the best action with the current state. """

        maxQ = self.get_maxQ(state)
        actions = self.get_maxQ_actions(state, maxQ)
    ...
    def createQ(self, state):
        ...
        if self.learning and state not in self.Q:
            self.Q[state] = {valid_action: 0.0 for valid_action in self.valid_actions}
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            ...
            if probability_roll <= self.epsilon:
                # random action
                action = random.choice(self.valid_actions)
            else:
                # choose action with highest Q-value
                action = self.select_maxQ_action(state)
        else:
            action = random.choice(self.valid_actions)
